 ...
 */
void call_default_set_baserel_rows_estimate(PlannerInfo* root, RelOptInfo* rel) {
 ...
 */
void call_default_set_joinrel_size_estimates(PlannerInfo* root, RelOptInfo* rel, RelOptInfo* outer_rel, RelOptInfo* inner_rel, SpecialJoinInfo* sjinfo, List* restrictlist) {
...
void aqo_set_baserel_rows_estimate(PlannerInfo* root, RelOptInfo* rel) {
    ...
    // elog(INFO, "        --->Fang Aqo Hash Join %lf", predicted);
    card_num += 1;
    predicted = predict_for_relation_learnedmodel();
    elog(INFO, "        --->Fang want to replace with learned card %ld => %lf", card_num, predicted);
    ...
    if (predicted >= 0) {
        ...
        elog(INFO, "        --->Fang want to learned card does not make sense, so call defualt estimation");
    }
...
void ppi_hook(ParamPathInfo* ppi) {
 ...
 * passes them to predict_for_relation.
 */
void aqo_set_joinrel_size_estimates(PlannerInfo* root, RelOptInfo* rel, RelOptInfo* outer_rel, RelOptInfo* inner_rel, SpecialJoinInfo* sjinfo, List* restrictlist) {
    double predicted;
    List* relids;
    List* outer_clauses;
    List* inner_clauses;
